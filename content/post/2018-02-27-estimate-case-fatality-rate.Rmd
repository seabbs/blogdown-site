...
This is a quick post exploring estimates of the case fatality ratio for Tuberculosis (TB) from [data](http://www.who.int/tb/country/data/download/en/) published by the World Health Organisation (WHO). It makes use of [getTBinR](https://www.samabbott.co.uk/getTBinR) (which is now on [CRAN](https://cran.r-project.org/web/packages/getTBinR/index.html)), [pacman](https://github.com/trinker/pacman) for package management, [hrbrthemes](https://github.com/hrbrmstr/hrbrthemes) for plot themes, and [pathwork](https://github.com/thomasp85/patchwork) for combining multiple plots into a storyboard. For an introduction to using getTBinR to explore the WHO TB data see [this](https://www.samabbott.co.uk/post/intro-gettbinr/) post.
...
Finally we summarise this post using `patchwork`, a great new package by [Thomas Pederson](https://twitter.com/thomasp85), which lets you easily combine ggplot plots into a single plot. The resulting storyboard (below) neatly summarises the exploratory plots, and the narrative they provide, that we have made in this post. See [here](https://www.samabbott.co.uk/post/2018-02-27-estimate-case-fatality-rate_files/figure-html/pathwork-storyboard-1.png) for a full size version.

```{r pathwork-storyboard, fig.width = 24, fig.height = 16, dpi = 330, message = FALSE}
storyboard <- (mp1 | mp2 ) /
    {
      plot_region_case_fatality + {
        plot_high_cfr +
