...
```{r map-cluster-membership, fig.height = 8, fig.width = 8, dpi = 330}
## Some issues here with extracting code from the sp file
## Solvied using the folling maptools functions - improvements appreciated!
gpclibPermit()
...
cluster_map <- ggplot(tb_cluster_results, 
                 aes(x = long, 
                     y = lat,
                     fill = cluster)) +
    geom_path(aes(group = group), alpha = 0.3) +
    geom_polygon(aes(group = group, fill = NULL), alpha = 0.1) +
