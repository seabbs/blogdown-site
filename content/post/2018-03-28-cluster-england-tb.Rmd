...
Next, we now need to check the completeness of the data. Ideally, we would use the most recent year of data for our clustering analysis but this may not be possible if variables are highly missing. The following function calculates the proportion of missing data in each year for each variable. 
...
knitr::include_graphics("../../static/img/fingertips/map-tb-clust.png")
```
...
```{r storyboard, fig.height = 8, fig.width = 8, dpi = 330, warning = FALSE}
plot_tb_cluster_map <- tb_cluster_map(tb_pca_pam)
...
knitr::include_graphics("../../static/img/fingertips/storyboard-fingertips-tb-clust.png")
```
